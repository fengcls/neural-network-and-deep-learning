...
from hw4_utils import shared_dataset,contextwin
...
class RNN(object):
    ...
    def train(self, x, y, learning_rate):
        x=numpy.asarray(x).astype(theano.config.floatX)
        self.train_sub(x, y, learning_rate)


def test_mlp_parity(nbit=8,learning_rate=0.1, L1_reg=0.00, L2_reg=0.0001, n_epochs=100,
             batch_size=128, n_hidden=500, n_hiddenLayers=3,
             verbose=True,load_file=True):
    # generate datasets
    if load_file:
        [train_set,valid_set,test_set]=numpy.load('sets'+str(nbit)+'.npy')
    else:
        train_set = gen_parity_pair(nbit, 1000)
        valid_set = gen_parity_pair(nbit, 500)
        test_set  = gen_parity_pair(nbit, 100)
    # numpy.save('sets12.npy',[train_set,valid_set,test_set])
...
def test_rnn_parity(**kwargs):
    ...
    # generate datasets
    if param['load_file']:
        [train_set,valid_set,test_set]=numpy.load('sets'+str(param['nbit'])+'.npy')
    else:
        train_set = gen_parity_pair(param['nbit'], 10000)
        valid_set = gen_parity_pair(param['nbit'], 500)
        test_set  = gen_parity_pair(param['nbit'], 100)
        # numpy.save('sets4.npy',[train_set,valid_set,test_set])
