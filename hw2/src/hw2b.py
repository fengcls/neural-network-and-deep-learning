...
class myMLP(object):
    ...
    still assume the same hidden unit number across hidden layers    
    ...
    def __init__(self, rng, input, n_in, n_hidden_unit, n_hidden_layer, n_out,activation):
        """Initialize the parameters for the multilayer perceptron
        ...
        :type n_hidden_unit: int
        :param n_hidden_unit: number of hidden units
        
        :type n_hidden_layer: int
        :param n_hidden_layer: int
        ...
        # or use append() in list object as done in
        # http://www.deeplearning.net/tutorial/DBN.html?highlight=hiddenlayer
        self.hiddenLayer = [None]*n_hidden_layer
        ...
        if (n_hidden_layer>1):
            for khl in range(1,n_hidden_layer):
                self.hiddenLayer[khl] = HiddenLayer(
                    rng=rng,
                    input=self.hiddenLayer[khl-1].output,
                    n_in=n_hidden_unit,
                    n_out=n_hidden_unit,
                    activation=activation
                )
        ...
        # of the hidden layer
        self.logRegressionLayer = LogisticRegression(
            input=self.hiddenLayer[n_hidden_layer-1].output,
            n_in=n_hidden_unit,
            n_out=n_out
        )
        # L1 norm ; one regularization option is to enforce L1 norm to
        ...
        for khl in range(n_hidden_layer):
            self.L1 = self.L1 + abs(self.hiddenLayer[khl].W).sum()

        # square of L2 norm ; one regularization option is to enforce
        # square of L2 norm to be small
        self.L2_sqr = (self.logRegressionLayer.W ** 2).sum()
        for khl in range(n_hidden_layer):
            self.L2_sqr = self.L2_sqr + (self.hiddenLayer[khl].W ** 2).sum()
        ...
        for khl in range(n_hidden_layer):
            self.params = self.hiddenLayer[khl].params + self.params
        # keep track of model input
        self.input = input
...
def test_mlp(learning_rate, L1_reg, L2_reg, n_epochs,
             batch_size, n_hidden_unit, n_hidden_layer, verbose, activation):
#def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
#             batch_size=50, n_hidden_unit=200, n_hidden_layer=3, verbose=False, activation=T.tanh):
    """
    ...
    """
    print(learning_rate, L1_reg, L2_reg, n_epochs,
             batch_size, n_hidden_unit, n_hidden_layer, verbose, activation)
...
if __name__ == '__main__':
    test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0005, n_epochs=1000,
